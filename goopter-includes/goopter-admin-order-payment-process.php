...
class Goopter_Admin_Order_Payment_Process {
    ...
    public function goopter_add_meta_box($post_type, $post_or_order_object) {
        ...
        if (goopter_is_active_screen($screen)) {
            add_meta_box('goopter_admin_order_payment_process', __('Reference Transaction', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), array($this, 'admin_order_payment_process'), $screen, 'side', 'default');
            add_meta_box('goopter_admin_order_reference_order', __('Reference Transaction', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), array($this, 'admin_order_reference_order'), $screen, 'side', 'default');
        }
    }
    ...
    public function goopter_get_reason_why_process_reference_transaction_button_not_available($order) {
        ...
        if ($this->goopter_is_order_payment_method_selected($order) == false) {
            $reason_array[] = __('Payment method is not available for payment process, Please select Payment method from Billing details section.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
        } else {
            if (empty($token_id) && $this->goopter_is_order_user_selected($order) == true) {
                $reason_array[] = __('Payment Token Or Reference transaction ID is not available for payment process.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
            }
        }
        if ($this->goopter_is_order_need_payment($order) == false) {
            $reason_array[] = __('Order total must be greater than zero to process a reference transaction.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
        }
        $reason_array[] = __("Make any necessary adjustments to the item(s) on the order and calculate totals.  Remember to click Update if any adjustments were made, and then click Process Reference Transaction.", 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
        return $reason_array;
    }
    ...
    public function goopter_get_reason_why_create_reference_transaction_order_button_not_available($order) {
        ...
        if ($this->goopter_is_order_user_selected($order) == false) {
            $reason_array[] = __('This order is not associated with a registered user account, hence a reference transaction can not be done.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
        }
        if ($this->goopter_is_order_payment_method_selected($order) == false) {
            $reason_array[] = __('Payment method is not available for payment process, Please select Payment method from Billing details section.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
        } else {
