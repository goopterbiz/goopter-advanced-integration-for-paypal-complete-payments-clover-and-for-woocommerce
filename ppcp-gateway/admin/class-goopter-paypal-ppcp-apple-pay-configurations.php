...
class Goopter_PayPal_PPCP_Apple_Pay_Configurations
{
    ...
    public function listApplePayDomain($returnRawResponse = false)
    {
        ...
        if (isset($response['total_items'])) {
            ...
            $jsonResponse['message'] = __('Domain listing retrieved successfully.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
        } else {
        ...
        try {
            ...
            if ($checkIsDomainAdded) {
                $successMessage = __('Your domain has been registered successfully, Close the popup and refresh the page to update the status.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
    ...
    public function registerDomain($domainNameToRegister)
    {
        if (!filter_var($domainNameToRegister, FILTER_VALIDATE_DOMAIN)) {
            throw new Exception(esc_html__('Please enter a valid domain name to register.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'));
        }
        $domainParams = [
        ...
        if (isset($response['domain'])) {
            ...
            return [
                ...
                'message' => __('Domain has been added successfully.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'),
            ...
            if (str_contains($message, 'DOMAIN_ALREADY_REGISTERED')) {
                $message = __('Domain is already registered.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
            } elseif (str_contains($message, 'DOMAIN_REGISTERED_WITH_ANOTHER_MERCHANT')) {
                $message = __('Domain is registered with another merchant.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
            }
            return ['status' => false, 'message' => __('An error occurred.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce') . "\n\n" . $message];
        }
    }
    ...
    public function removeDomain($domainNameToRemove): array
    {
        ...
        if (isset($response['domain'])) {
            ...
            return [
                'status' => true,
                'message' => __('Domain has been removed successfully.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce')
            ];
        } else {
            $this->payment_request->error_email_notification = false;
            $message = $this->payment_request->goopter_ppcp_get_readable_message($response);
    ...
    private function addDomainValidationFiles()
    {
        ...
        try {
            ...
            throw new Exception(esc_html__("Unable to update the verification file content. Error: ", 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce') . esc_html($exception->getMessage()));
        }
