...
class Goopter_PayPal_PPCP_Payment {
    ...
    public $goopter_ppcp_used_payment_method;
    ...
    public function goopter_ppcp_load_class() {
        try {
            ...
            if (!class_exists('Goopter_PayPal_PPCP_Error')) {
                include_once PAYPAL_FOR_WOOCOMMERCE_PLUGIN_DIR . '/ppcp-gateway/class-goopter-paypal-ppcp-error.php';
            }
            if (!class_exists('Goopter_PayPal_PPCP_Front_Action')) {
    ...
    private function add_nonce_in_response(array $response): array {
        ...
        if (Goopter_PayPal_PPCP_Front_Action::$is_user_logged_in_before_checkout != $current_login_status) {
    ...
    public function goopter_ppcp_create_order_request($woo_order_id = null) {
        try {
            ...
            if ($woo_order_id == null) {
                $cart = $this->goopter_ppcp_get_details_from_cart();
            } else {
                $cart = $this->goopter_ppcp_get_details_from_order($woo_order_id);
            }
            ...
            if (!empty($payment_method)) {
                $payment_method_title = goopter_ppcp_get_payment_method_title($payment_method);
            ...
            $currency_code = apply_filters('goopter_ppcp_woocommerce_currency', goopter_ppcp_get_currency($woo_order_id), $cart['order_total']);
            $body_request = array(
                'intent' => $intent,
                'application_context' => $this->goopter_ppcp_application_context(),
            ...
            if ($woo_order_id != null) {
                ...
                $body_request['purchase_units'][0]['custom_id'] = apply_filters('goopter_ppcp_custom_id', $this->invoice_prefix . str_replace("#", "", $order->get_order_number()), $order);
            } else {
                $body_request['purchase_units'][0]['invoice_id'] = $reference_id;
                $body_request['purchase_units'][0]['custom_id'] = apply_filters('goopter_ppcp_custom_id', $reference_id, '');
            }
            $country_code = "";
            $full_name = "";
            ...
            $body_request['purchase_units'][0]['soft_descriptor'] = goopter_ppcp_get_value('soft_descriptor', $this->soft_descriptor);
            ...
            if ($this->send_items === true) {
                if (isset($cart['total_item_amount']) && $cart['total_item_amount'] > 0) {
                    $body_request['purchase_units'][0]['amount']['breakdown']['item_total'] = array(
                        'currency_code' => apply_filters('goopter_ppcp_woocommerce_currency', goopter_ppcp_get_currency($woo_order_id), $cart['total_item_amount']),
                ...
                if (isset($cart['shipping']) && $cart['shipping'] > 0) {
                    $body_request['purchase_units'][0]['amount']['breakdown']['shipping'] = array(
                        'currency_code' => apply_filters('goopter_ppcp_woocommerce_currency', goopter_ppcp_get_currency($woo_order_id), $cart['shipping']),
                        'value' => $cart['shipping'],
                    );
                }
                if (isset($cart['ship_discount_amount']) && $cart['ship_discount_amount'] > 0) {
                    $body_request['purchase_units'][0]['amount']['breakdown']['shipping_discount'] = array(
                        'currency_code' => apply_filters('goopter_ppcp_woocommerce_currency', goopter_ppcp_get_currency($woo_order_id), goopter_ppcp_round($cart['ship_discount_amount'], $decimals)),
                        'value' => goopter_ppcp_round($cart['ship_discount_amount'], $decimals),
                    );
                }
                if (isset($cart['order_tax']) && $cart['order_tax'] > 0) {
                    $body_request['purchase_units'][0]['amount']['breakdown']['tax_total'] = array(
                        'currency_code' => apply_filters('goopter_ppcp_woocommerce_currency', goopter_ppcp_get_currency($woo_order_id), $cart['order_tax']),
                        'value' => $cart['order_tax'],
                    );
                }
                if (isset($cart['discount']) && $cart['discount'] > 0) {
                    $body_request['purchase_units'][0]['amount']['breakdown']['discount'] = array(
                        'currency_code' => apply_filters('goopter_ppcp_woocommerce_currency', goopter_ppcp_get_currency($woo_order_id), $cart['discount']),
                        'value' => $cart['discount'],
                    );
                }
                if (isset($cart['items']) && !empty($cart['items'])) {
                    foreach ($cart['items'] as $key => $order_items) {
                        ...
                        $body_request['purchase_units'][0]['items'][$key] = array(
                            ...
                            'unit_amount' =>
                            array(
                                'currency_code' => apply_filters('goopter_ppcp_woocommerce_currency', goopter_ppcp_get_currency($woo_order_id), $order_items['amount']),
            ...
            if ($woo_order_id != null) {
                ...
                $goopter_ppcp_payment_method_title = Goopter_Session_Manager::get('payment_method_title', false);
                ...
                $goopter_ppcp_used_payment_method = Goopter_Session_Manager::get('used_payment_method', false);
            ...
            if ($this->goopter_ppcp_used_payment_method === 'venmo') {
            ...
            if (goopter_ppcp_is_save_payment_method($this->enable_tokenized_payments)) {
            ...
            } elseif ($this->goopter_ppcp_used_payment_method === 'venmo') {
                if (isset($body_request['purchase_units'][0]['shipping']['address'])) {
                    ...
                    $body_request['payment_source']['venmo']['experience_context']['shipping_preference'] = $this->goopter_ppcp_shipping_preference();
                }
                unset($body_request['application_context']);
            }
            $body_request = goopter_ppcp_remove_empty_key($body_request);
            $args = array(
                'method' => 'POST',
                'headers' => array('Content-Type' => 'application/json', 'Authorization' => '', "prefer" => "return=representation", 'PayPal-Request-Id' => $this->generate_request_id(), 'Paypal-Auth-Assertion' => $this->goopter_ppcp_paypalauthassertion()),
                'body' => $body_request
            );
            $args = apply_filters('goopter_ppcp_request_args', $args, 'create_order', $woo_order_id);
            ...
            if (!empty($this->api_response['status'])) {
                ...
                $errorMessage = $this->goopter_ppcp_get_readable_message($this->api_response, $error_email_notification_param);
                ...
                if (str_contains($errorMessage, 'CURRENCY_NOT_SUPPORTED')) {
                    ...
                    wp_send_json_error(sprintf(__('Currency code (%s) is not currently supported.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), $currency_code));
                } else {
                    wp_send_json_error(__('We were unable to process your order, please try again with same or other payment method(s).', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'));
                }
            }
        } catch (Exception $ex) {
            $this->api_log->log("The exception was created on line: " . $ex->getFile() . ' ' . $ex->getLine(), 'error');
    ...
    public function goopter_ppcp_get_discount_amount_from_cart_item() {
        $cart_item_discount_amount = 0;
        $decimals = $this->goopter_ppcp_get_number_of_decimal_digits();
        foreach (WC()->cart->cart_contents as $cart_item_key => $values) {
            $amount = goopter_ppcp_round($values['line_subtotal'] / $values['quantity'], $decimals);
            if ($amount < 0) {
                $cart_item_discount_amount += goopter_ppcp_round($amount * $values['quantity'], $decimals);
            }
        }
        foreach (WC()->cart->get_fees() as $cart_item_key => $fee_values) {
            if ($fee_values->amount < 0) {
                $cart_item_discount_amount += goopter_ppcp_round($fee_values->amount * 1, $decimals);
            }
        }
        return absint($cart_item_discount_amount);
    }

    public function goopter_ppcp_get_details_from_cart() {
        try {
            $decimals = $this->goopter_ppcp_get_number_of_decimal_digits();
            $rounded_total = $this->goopter_ppcp_get_rounded_total_in_cart();
            $discounts = WC()->cart->get_cart_discount_total();
            $cart_item_discount_amount = $this->goopter_ppcp_get_discount_amount_from_cart_item();
            ...
            $items = $this->goopter_ppcp_get_paypal_line_items_from_cart();
            ...
            if (function_exists("scd_get_target_currency") && $multicurrency_payment) {
                ...
                $rounded_total = goopter_ppcp_round($rounded_total * $rate_c, $decimals);
                $discounts = $discounts * $rate_c;
                foreach ($items as $key => $item) {
                    $items[$key]['amount'] = goopter_ppcp_round($item['amount'] * $rate_c, $decimals);
                }
            }
            /**
             ...
             */
            $details = array(
                'total_item_amount' => goopter_ppcp_round($cart_contents_total, $decimals),
                'order_tax' => goopter_ppcp_round($order_tax, $decimals),
                'shipping' => goopter_ppcp_round($shipping_total, $decimals),
                'items' => $items,
                'shipping_address' => $this->goopter_ppcp_get_address_from_customer(),
                'email' => WC()->customer->get_billing_email(),
            );
            if ((float) $details['total_item_amount'] == 0) {
                $details['total_item_amount'] = WC()->cart->fee_total;
            }
            return $this->goopter_ppcp_get_details($details, $discounts, $rounded_total, $cart_total);
        } catch (Exception $ex) {
            $this->api_log->log("The exception was created on line: " . $ex->getFile() . ' ' . $ex->getLine(), 'error');
            $this->api_log->log($ex->getMessage(), 'error');
        }
    ...
    public function getOrderLineItems(WC_Order $order): array {
        ...
        foreach ($order->get_items() as $item) {
            $lineItems[] = [
                'label' => $item->get_name(''),
                'amount' => goopter_ppcp_round($order->get_line_total($item), $decimals)
            ];
        }
        ...
        if ($order->needs_shipping_address()) {
            $lineItems[] = [
                'label' => 'Shipping',
                'amount' => goopter_ppcp_round($order->get_shipping_total(''), $decimals)
            ];
        }
        ...
        foreach ($order->get_fees() as $item) {
            ...
            $lineItems[] = [
                'label' => $item->get_name(''),
                'amount' => goopter_ppcp_round($amount, $decimals)
            ];
        }
        ...
        if ($tax > 0) {
            $lineItems[] = [
                'label' => 'Tax',
                'amount' => goopter_ppcp_round($tax, $decimals)
            ];
        }
    ...
    public function getCartLineItems(): array {
        $lineItems = [];
        $details = $this->goopter_ppcp_get_details_from_cart();
        ...
        foreach ($cart as $cart_item) {
            $lineItems[] = [
                ...
                'amount' => goopter_ppcp_round($cart_item['data']->get_price(), $decimals)
            ];
        }
        ...
        if (WC()->cart->needs_shipping()) {
            $lineItems[] = [
                'label' => 'Shipping',
                'amount' => goopter_ppcp_round($details['shipping'], $decimals)
            ];
        }

        foreach (WC()->cart->get_fees() as $item) {
            $lineItems[] = [
                'label' => html_entity_decode(wc_trim_string($item->name ?: __('Fee', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), 127), ENT_NOQUOTES, 'UTF-8'),
    ...
    public function goopter_ppcp_get_number_of_decimal_digits() {
        try {
            return $this->goopter_ppcp_is_currency_supports_zero_decimal() ? 0 : 2;
        } catch (Exception $ex) {
            $this->api_log->log("The exception was created on line: " . $ex->getFile() . ' ' . $ex->getLine(), 'error');
            $this->api_log->log($ex->getMessage(), 'error');
        }
    ...
    public function goopter_ppcp_is_currency_supports_zero_decimal() {
        try {
    ...
    public function goopter_ppcp_get_rounded_total_in_cart() {
        try {
            $decimals = $this->goopter_ppcp_get_number_of_decimal_digits();
            $rounded_total = 0;
            foreach (WC()->cart->cart_contents as $cart_item_key => $values) {
                $amount = goopter_ppcp_round($values['line_subtotal'] / $values['quantity'], $decimals);
                if ($amount > 0) {
                    $rounded_total += goopter_ppcp_round($amount * $values['quantity'], $decimals);
                }
            }
            foreach (WC()->cart->get_fees() as $cart_item_key => $fee_values) {
                if ($fee_values->amount > 0) {
                    $rounded_total += goopter_ppcp_round($fee_values->amount * 1, $decimals);
                }
            }
            return goopter_ppcp_round($rounded_total, $decimals);
        } catch (Exception $ex) {
            $this->api_log->log("The exception was created on line: " . $ex->getFile() . ' ' . $ex->getLine(), 'error');
            $this->api_log->log($ex->getMessage(), 'error');
        }
    ...
    public function goopter_ppcp_get_paypal_line_items_from_cart() {
        try {
            $decimals = $this->goopter_ppcp_get_number_of_decimal_digits();
            $items = array();
            foreach (WC()->cart->cart_contents as $cart_item_key => $values) {
                ...
                $product_name = apply_filters('goopter_ppcp_product_name', $product_name, $product, $desc, $values);
                ...
                if ($amount > 0) {
                    $item = array(
                        'name' => $product_name,
                        'description' => apply_filters('goopter_ppcp_product_description', $desc),
                        'sku' => $sku,
                        'category' => $category,
            ...
            foreach (WC()->cart->get_fees() as $cart_item_key => $fee_values) {
                ...
                if ($amount > 0) {
                    $fee_item = array(
                        'name' => html_entity_decode(wc_trim_string($fee_values->name ? $fee_values->name : __('Fee', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), 127), ENT_NOQUOTES, 'UTF-8'),
                        'description' => '',
                        'quantity' => 1,
                        'amount' => Goopter_Gateway_Paypal::number_format($fee_values->amount),
                    );
    ...
    public function goopter_ppcp_get_address_from_customer() {
        try {
    ...
    public function goopter_ppcp_get_details($details, $discounts, $rounded_total, $total) {
        try {
            $decimals = $this->goopter_ppcp_get_number_of_decimal_digits();
            $discounts = goopter_ppcp_round($discounts, $decimals);
            $details['order_total'] = goopter_ppcp_round(
            ...
            if ($details['total_item_amount'] != $rounded_total) {
                ...
                if ($diff > 0.000001 && 0.0 !== (float) $diff) {
                    $extra_line_item = $this->goopter_ppcp_get_extra_offset_line_item($diff);
                    $details['items'][] = $extra_line_item;
                    $details['total_item_amount'] += $extra_line_item['amount'];
                    $details['total_item_amount'] = goopter_ppcp_round($details['total_item_amount'], $decimals);
                    $details['order_total'] += $extra_line_item['amount'];
                    $details['order_total'] = goopter_ppcp_round($details['order_total'], $decimals);
                }
            }
            ...
            $wc_order_total = goopter_ppcp_round($total, $decimals);
            $discounted_total = goopter_ppcp_round($details['order_total'], $decimals);
            if ($wc_order_total != $discounted_total) {
                if ($discounted_total < $wc_order_total) {
                    $details['order_tax'] += $wc_order_total - $discounted_total;
                    $details['order_tax'] = goopter_ppcp_round($details['order_tax'], $decimals);
                } else {
                    $details['ship_discount_amount'] += $wc_order_total - $discounted_total;
                    $details['ship_discount_amount'] = goopter_ppcp_round($details['ship_discount_amount'], $decimals);
            ...
            if (abs($lisum) > 0.000001 && 0.0 !== (float) $diff) {
                $details['items'][] = $this->goopter_ppcp_get_extra_offset_line_item($details['total_item_amount'] - $lisum);
            }
            return $details;
        } catch (Exception $ex) {
            $this->api_log->log("The exception was created on line: " . $ex->getFile() . ' ' . $ex->getLine(), 'error');
    ...
    public function goopter_ppcp_get_extra_offset_line_item($amount) {
        try {
            $decimals = $this->goopter_ppcp_get_number_of_decimal_digits();
            return array(
    ...
    public function goopter_ppcp_application_context($return_url = false) {
        $smart_button = Goopter_PayPal_PPCP_Smart_Button::instance();
        $application_context = array(
            'brand_name' => $this->brand_name,
            'locale' => 'en-US',
            'landing_page' => $this->landing_page,
            'shipping_preference' => $this->goopter_ppcp_shipping_preference(),
            'user_action' => $smart_button->goopter_ppcp_is_skip_final_review() ? 'PAY_NOW' : 'CONTINUE'
        );
        if ($this->checkout_disable_smart_button === true || $return_url === true) {
            $application_context['return_url'] = add_query_arg(array('goopter_ppcp_action' => 'regular_capture', 'utm_nooverride' => '1'), untrailingslashit(WC()->api_request_url('Goopter_PayPal_PPCP_Front_Action')));
            $application_context['cancel_url'] = add_query_arg(array('goopter_ppcp_action' => 'regular_cancel', 'utm_nooverride' => '1'), untrailingslashit(WC()->api_request_url('Goopter_PayPal_PPCP_Front_Action')));
        }
    ...
    public function goopter_ppcp_set_payer_details($woo_order_id, $body_request) {
        if ($woo_order_id != null) {
            $order = wc_get_order($woo_order_id);
    ...
    public function generate_request_id() {
        ...
        return $addr . $pid . (isset($_SERVER['REQUEST_TIME']) 
            ? sanitize_text_field(wp_unslash($_SERVER['REQUEST_TIME'])) 
            : time()) . wp_rand(0, 0xffff);
    }
    ...
    public function goopter_ppcp_get_readable_message($error, $error_email_notification_param = array()) {
        ...
        if ($this->error_email_notification) {
            $this->goopter_ppcp_error_email_notification($error_email_notification_param, $message);
        }
        return $message;
    }
     ...
     *
     * We already have another method goopter_ppcp_get_checkout_details but the response returned by that is not proper
     * due to decoding the object and other places its being used
     * @param $paypal_order_id
     * @return false|mixed
     */
    public function goopter_ppcp_get_paypal_order($paypal_order_id) {
        try {
            $args = array(
                'timeout' => 60,
                'redirection' => 5,
    ...
    public function goopter_ppcp_get_checkout_details($paypal_order_id) {
        try {
            $args = array(
                'timeout' => 60,
                'redirection' => 5,
    ...
    public function get_set_payment_method_title_from_session($woo_order_id = null) {
        ...
        $payment_method_id = Goopter_Session_Manager::get('payment_method_id');
        if (!empty($payment_method_id)) {
            $order->set_payment_method($payment_method_id);
            $order->save();
        }
        if (!empty($goopter_ppcp_payment_method_title) && !empty($woo_order_id)) {
            $order->set_payment_method_title($goopter_ppcp_payment_method_title);
            $order->save();
        } else {
            if (is_null($goopter_ppcp_payment_method_title) && $payment_method_id == 'goopter_ppcp_cc'){
                $goopter_ppcp_payment_method_title = $this->titles;
            } else {
                $goopter_ppcp_payment_method_title = $this->title;
            }
        }
        return $goopter_ppcp_payment_method_title;
    }
    ...
    public function goopter_ppcp_order_capture_request($woo_order_id, $need_to_update_order = true) {
        try {
            $order = wc_get_order($woo_order_id);
            if ($need_to_update_order) {
                $bool = $this->goopter_ppcp_update_order($order);
                if($bool === false) {
                    return false;
                }
            }
            ...
            $goopter_ppcp_payment_method_title = $this->get_set_payment_method_title_from_session($woo_order_id);
            if (!empty($goopter_ppcp_payment_method_title)) {
                $order->set_payment_method_title($goopter_ppcp_payment_method_title);
            }
            $payment_method_id = Goopter_Session_Manager::get('payment_method_id', false);
            ...
            if (isset($this->api_response['id']) && !empty($this->api_response['id'])) {
                do_action('goopter_ppcp_order_data', $this->api_response, $woo_order_id);
                $order->update_meta_data('_paypal_order_id', $this->api_response['id']);
                $order->save_meta_data();
                if ($this->api_response['status'] == 'COMPLETED') {
                    if (isset($this->api_response['payment_source']['card']['attributes']['vault']['status']) && 'APPROVED' === $this->api_response['payment_source']['card']['attributes']['vault']['status']) {
                        ...
                        if (!empty($api_response['id'])) {
                            ...
                            $this->subscriptions_helper->goopter_ppcp_wc_save_payment_token($woo_order_id, $api_response);
                        }
                        ...
                        $this->subscriptions_helper->goopter_ppcp_wc_save_payment_token($woo_order_id, $this->api_response);
                    ...
                    if (!empty($payment_source['card'])) {
                        $card_response_order_note = __('Card Details', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
                        $card_response_order_note .= "\n";
                        $card_response_order_note .= 'Last digits : ' . $payment_source['card']['last_digits'];
                        $card_response_order_note .= "\n";
                        $card_response_order_note .= 'Brand : ' . goopter_ppcp_readable($payment_source['card']['brand']);
                        $card_response_order_note .= "\n";
                        $card_response_order_note .= 'Card type : ' . goopter_ppcp_readable($payment_source['card']['type']);
                    ...
                    foreach ($this->api_response['purchase_units'] as $captures_key => $captures) {
                        ...
                        if (!empty($processor_response['avs_code'])) {
                            $avs_response_order_note = __('Address Verification Result', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
                        ...
                        if (!empty($processor_response['cvv_code'])) {
                            $cvv2_response_code = __('Card Security Code Result', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
                            $cvv2_response_code .= "\n";
                        ...
                        if (!empty($processor_response['response_code'])) {
                            $response_code = __('Processor response code Result', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
                            $response_code .= "\n";
                        ...
                        $this->goopter_ppcp_payment_status_woo_order_note($woo_order_id, $payment_status, $payment_status_reason);
                    ...
                    if (count($this->api_response['purchase_units']) === 1) {
                        if ($payment_status == 'COMPLETED') {
                            ...
                            $order->add_order_note(sprintf(__('Payment via %1$s: %2$s.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), $order->get_payment_method_title(), ucfirst(strtolower($payment_status))));
                        } else {
                    ...
                    if (!empty($processor_response['payment_advice_code'])) {
                        $payment_advice_code = __('Payment Advice Codes Result', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
                        $payment_advice_code .= "\n";
                    ...
                    if ($payment_status == 'COMPLETED') {
                        ...
                        $order->update_status('failed', sprintf(__('Payment via %s declined.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), $goopter_ppcp_payment_method_title));
                        $order->save();
                        wc_add_notice(__('Unfortunately your order cannot be processed as the originating bank/merchant has declined your transaction. Please attempt your purchase again.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), 'error');
                        return false;
                    } else {
                    ...
                    $order->add_order_note(sprintf(__('%1$s Capture Transaction ID: %2$s', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), $goopter_ppcp_payment_method_title, $transaction_id));
                ...
                wc_add_notice(__('This payment was unable to be processed successfully. Please try again with another payment method.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), 'error');
    ...
    public function goopter_ppcp_update_order($order) {
        try {
            $decimals = $this->goopter_ppcp_get_number_of_decimal_digits();
            $patch_request = array();
            $reference_id = Goopter_Session_Manager::get('reference_id');
            $order_id = $order->get_id();
            $paypal_order_id = Goopter_Session_Manager::get('paypal_order_id');
            if (empty($paypal_order_id)) {
                $paypal_order_id = goopter_ppcp_get_post_meta($order_id, '_paypal_order_id');
                if (empty($paypal_order_id)) {
                    goopter_session_expired_exception('_paypal_order_id missing in the update_order call.');
                }
            }
            // Note: This could create issue with old orders where we don't have reference_id
            if (empty($reference_id)) {
                $reference_id = goopter_ppcp_get_post_meta($order_id, '_paypal_reference_id');
                if (empty($reference_id)) {
                    goopter_session_expired_exception('_paypal_reference_id is missing in the update_order call.');
                }
            }
            $cart = $this->goopter_ppcp_get_details_from_order($order_id);
            if (isset($cart['total_item_amount']) && $cart['total_item_amount'] > 0) {
                $update_amount_request['item_total'] = array(
                    'currency_code' => goopter_ppcp_get_currency($order_id),
                    'value' => $cart['total_item_amount'],
                );
            }
            if (isset($cart['discount']) && $cart['discount'] > 0) {
            ...
            $purchase_units = array(
                ...
                'amount' =>
                array(
                    'currency_code' => apply_filters('goopter_ppcp_woocommerce_currency', goopter_ppcp_get_currency($order_id), $cart['order_total']),
                    'value' => $cart['order_total'],
                    'breakdown' => array()
                )
            );
            $purchase_units['invoice_id'] = $this->invoice_prefix . str_replace("#", "", $order->get_order_number());
            $purchase_units['custom_id'] = apply_filters('goopter_ppcp_custom_id', $this->invoice_prefix . str_replace("#", "", $order->get_order_number()), $order);
            $purchase_units['payee']['merchant_id'] = $this->merchant_id;
            if ($this->send_items === true) {
                if (isset($cart['total_item_amount']) && $cart['total_item_amount'] > 0) {
                    $purchase_units['amount']['breakdown']['item_total'] = array(
                        'currency_code' => apply_filters('goopter_ppcp_woocommerce_currency', goopter_ppcp_get_currency($order_id), $cart['total_item_amount']),
                        'value' => $cart['total_item_amount'],
                    );
                }
                if (isset($cart['shipping']) && $cart['shipping'] > 0) {
                    $purchase_units['amount']['breakdown']['shipping'] = array(
                        'currency_code' => apply_filters('goopter_ppcp_woocommerce_currency', goopter_ppcp_get_currency($order_id), $cart['shipping']),
                        'value' => $cart['shipping'],
                    );
                }
                if (isset($cart['ship_discount_amount']) && $cart['ship_discount_amount'] > 0) {
                    $purchase_units['amount']['breakdown']['shipping_discount'] = array(
                        'currency_code' => apply_filters('goopter_ppcp_woocommerce_currency', goopter_ppcp_get_currency($order_id), goopter_ppcp_round($cart['ship_discount_amount'], $decimals)),
                        'value' => goopter_ppcp_round($cart['ship_discount_amount'], $decimals),
                    );
                }
                if (isset($cart['order_tax']) && $cart['order_tax'] > 0) {
                    $purchase_units['amount']['breakdown']['tax_total'] = array(
                        'currency_code' => apply_filters('goopter_ppcp_woocommerce_currency', goopter_ppcp_get_currency($order_id), $cart['order_tax']),
                        'value' => $cart['order_tax'],
                    );
                }
                if (isset($cart['discount']) && $cart['discount'] > 0) {
                    $purchase_units['amount']['breakdown']['discount'] = array(
                        'currency_code' => apply_filters('goopter_ppcp_woocommerce_currency', goopter_ppcp_get_currency($order_id), $cart['discount']),
                        'value' => $cart['discount'],
                    );
                }
                ...
                if (isset($cart['items']) && !empty($cart['items'])) {
                    foreach ($cart['items'] as $key => $order_items) {
                        ...
                        $purchase_units['items'][$key] = array(
                            ...
                            'unit_amount' =>
                            array(
                                'currency_code' => apply_filters('goopter_ppcp_woocommerce_currency', goopter_ppcp_get_currency($order_id), $order_items['amount']),
                                'value' => $order_items['amount'],
                            ),
                        );
                    }
            ...
            $body_request = goopter_ppcp_remove_empty_key($purchase_units);
            $patch_request[] = array(
                'op' => 'replace',
            ...
            $patch_request = apply_filters('goopter_ppcp_request_args', $patch_request, 'update_order', $order_id);
            $args = array(
                'timeout' => 70,
                'method' => 'PATCH',
                'httpversion' => '1.1',
            ...
            $this->api_log->log("The exception was created on line: " . $ex->getFile() . ' ' . $ex->getLine(), 'error');
            $this->api_log->log($ex->getMessage(), 'error');
            wc_add_notice(__('Sorry, your session has expired.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'));
            return false;
        }
    }

    public function goopter_ppcp_get_details_from_order($order_id) {
        try {
            $order = wc_get_order($order_id);
            $decimals = $this->goopter_ppcp_is_currency_supports_zero_decimal() ? 0 : 2;
            $rounded_total = $this->goopter_ppcp_get_rounded_total_in_order($order);
            $details = array(
                'total_item_amount' => goopter_ppcp_round($order->get_subtotal(), $decimals),
                'order_tax' => goopter_ppcp_round($order->get_total_tax(), $decimals),
                'shipping' => goopter_ppcp_round($order->get_shipping_total(), $decimals),
                'items' => $this->goopter_ppcp_get_paypal_line_items_from_order($order),
            );
            if ((float) $details['total_item_amount'] == 0) {
                $details['total_item_amount'] = $order->get_total_fees();
            }
            $details = $this->goopter_ppcp_get_details($details, $order->get_total_discount(), $rounded_total, $order->get_total());
            return $details;
        } catch (Exception $ex) {
            $this->api_log->log("The exception was created on line: " . $ex->getFile() . ' ' . $ex->getLine(), 'error');
            $this->api_log->log($ex->getMessage(), 'error');
    ...
    public function goopter_ppcp_get_rounded_total_in_order($order) {
        try {
            $decimals = $this->goopter_ppcp_get_number_of_decimal_digits();
            $order = wc_get_order($order);
            $rounded_total = 0;
            foreach ($order->get_items() as $cart_item_key => $values) {
                $amount = goopter_ppcp_round($values['line_subtotal'] / $values['qty'], $decimals);
                if ($amount > 0) {
                    $rounded_total += goopter_ppcp_round($amount * $values['qty'], $decimals);
                }
            }
            foreach ($order->get_fees() as $cart_item_key => $fee_values) {
                $amount = $order->get_line_total($fee_values);
                if ($amount > 0) {
                    $rounded_total += goopter_ppcp_round($amount * 1, $decimals);
                }
            }
    ...
    public function goopter_ppcp_get_paypal_line_items_from_order($order) {
        try {
            $decimals = $this->goopter_ppcp_get_number_of_decimal_digits();
            $items = array();
            foreach ($order->get_items() as $cart_item_key => $values) {
            ...
            foreach ($order->get_fees() as $fee_values) {
                ...
                if ($amount > 0) {
                    $item = array(
                        'name' => html_entity_decode(wc_trim_string($fee_item_name ? $fee_item_name : __('Fee', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), 127), ENT_NOQUOTES, 'UTF-8'),
                        'description' => '',
                        'quantity' => 1,
    ...
    public function goopter_ppcp_refund_order($order_id, $amount, $reason, $transaction_id) {
        try {
            $order = wc_get_order($order_id);
            $decimals = $this->goopter_ppcp_get_number_of_decimal_digits();
            ...
            $currency_code = apply_filters('goopter_ppcp_woocommerce_currency', goopter_ppcp_get_currency($order_id), goopter_ppcp_round($amount, $decimals));
            if (!empty($amount) && $amount > 0) {
                $body_request['amount'] = array(
                    'value' => goopter_ppcp_round($amount, $decimals),
                    'currency_code' => $currency_code
                );
            }
            ...
            if (isset($this->api_response['status']) && $this->api_response['status'] == "COMPLETED") {
                ...
                $order->add_order_note(
                    // Translators: %1$s is the refunded amount with currency, %2$s is the refund transaction ID.
                        sprintf(__('Refunded %1$s - Refund ID: %2$s', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), wc_price($gross_amount, array('currency' => $currency_code)), $refund_transaction_id)
                );
                ...
                $order->add_order_note(sprintf(__('Payment via %1$s Pending. Pending reason: %2$s.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), $goopter_ppcp_payment_method_title, $pending_reason_text));
                $order->add_order_note(
                    // Translators: %1$s is the refunded amount with currency, %2$s is the refund transaction ID.
                        sprintf(__('Refund Amount %1$s - Refund ID: %2$s', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), wc_price($gross_amount, array('currency' => $currency_code)), $refund_transaction_id)
                );
            } else {
    ...
    public function goopter_ppcp_order_auth_request($woo_order_id) {
        try {
            $this->paymentaction = apply_filters('goopter_ppcp_paymentaction', $this->paymentaction, $woo_order_id);
            $order = wc_get_order($woo_order_id);
            $bool = $this->goopter_ppcp_update_order($order);
            ...
            if (!empty($this->api_response['id'])) {
                ...
                if ($this->api_response['status'] == 'COMPLETED' && strtolower($payment_status) != "denied") {
                    ...
                    if ($payment_status == 'COMPLETED') {
                        ...
                        $order->payment_complete($transaction_id);
                        // Translators: %1$s is the payment method title, %2$s is the payment status.
                        $order->add_order_note(sprintf(__('Payment via %1$s: %2$s.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), $goopter_ppcp_payment_method_title, ucfirst(strtolower($payment_status))));
                    } elseif ($payment_status === 'DECLINED') {
                        // Translators: %s is the payment method title.
                        $order->update_status('failed', sprintf(__('Payment via %s declined.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), $goopter_ppcp_payment_method_title));
                        wc_add_notice(__('Unfortunately your order cannot be processed as the originating bank/merchant has declined your transaction. Please attempt your purchase again.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), 'error');
                    ...
                    $order->add_order_note(sprintf(__('%1$s Authorization Transaction ID: %2$s', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), $goopter_ppcp_payment_method_title, $transaction_id));
                    $order->add_order_note('Seller Protection Status: ' . goopter_ppcp_readable($seller_protection));
                    ...
                    if ($this->is_auto_capture_auth) {
                        $order->add_order_note(__('Payment authorized. Change payment status to processing or complete to capture funds.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'));
                    }
                    $order->save();
                    return true;
                } else {
                    $response_code = __('Processor authorization status: ', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
    ...
    public function goopter_ppcp_show_details_authorized_payment($authorization_id) {
        try {
            $args = array(
                'timeout' => 60,
                'redirection' => 5,
    ...
    public function goopter_ppcp_void_authorized_payment($authorization_id, $note_to_payer = null) {
        try {
            $args = array(
                'method' => 'POST',
                'timeout' => 60,
            ...
            if (isset($this->api_response['status']) && strtolower($this->api_response['status']) == 'voided') {
                ...
                $error_message = $this->goopter_ppcp_get_readable_message($this->api_response);
    ...
    public function goopter_ppcp_capture_authorized_payment($woo_order_id) {
        try {
            $order = wc_get_order($woo_order_id);
            if ($order === false) {
                return false;
            ...
            $this->goopter_ppcp_capture_authorized_payment_admin($order, $order_data);
        } catch (Exception $ex) {
            $this->api_log->log("The exception was created on line: " . $ex->getFile() . ' ' . $ex->getLine(), 'error');
            $this->api_log->log($ex->getMessage(), 'error');
        }
    ...
    public function goopter_ppcp_update_woo_order_data($paypal_order_id) {
        $this->checkout_details = $this->goopter_ppcp_get_checkout_details($paypal_order_id);
        Goopter_Session_Manager::set('paypal_transaction_details', $this->checkout_details);
        if (empty($this->checkout_details)) {
            return false;
        }
        if (!empty($this->checkout_details)) {
            $shipping_details = goopter_ppcp_get_mapped_shipping_address($this->checkout_details);
            $billing_details = goopter_ppcp_get_mapped_billing_address($this->checkout_details);
            goopter_ppcp_update_customer_addresses_from_paypal($shipping_details, $billing_details);
        }
        $order_id = goopter_ppcp_get_awaiting_payment_order_id();
        $order = wc_get_order($order_id);
        $this->paymentaction = apply_filters('goopter_ppcp_paymentaction', $this->paymentaction, $order_id);
        $goopter_ppcp_payment_method_title = $this->get_set_payment_method_title_from_session($order_id);
        if ($this->paymentaction === 'capture' && !empty($this->checkout_details->status) && $this->checkout_details->status == 'COMPLETED' && $order !== false) {
            ...
            if (!empty($payment_status_reason)) {
                ...
                $order->add_order_note(sprintf(__('Payment via %1$s Pending. PayPal reason: %2$s.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), $goopter_ppcp_payment_method_title, $payment_status_reason));
            }
            $order->set_transaction_id($transaction_id);
            $order->update_meta_data('_payment_status', $payment_status);
            $order->update_meta_data('_auth_transaction_id', $transaction_id);
            ...
            // Translators: %1$s is the payment method title, %2$s is the authorization transaction ID.
            $order->add_order_note(sprintf(__('%1$s Authorization Transaction ID: %2$s', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), $this->title, $transaction_id));
            $order->add_order_note('Seller Protection Status: ' . goopter_ppcp_readable($seller_protection));
            if (class_exists('Goopter_PayPal_PPCP_Admin_Action')) {
                Goopter_PayPal_PPCP_Admin_Action::instance()->removeAutoCaptureHooks();
            }
            ...
            $order->save();
            $order->add_order_note(__('Payment authorized. Change order status to processing or complete for capture funds.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'));
        }
    }

    public function goopter_ppcp_paypalauthassertion() {
        $temp = array(
            "alg" => "none"
        );
    ...
    public function goopter_ppcp_get_generate_token() {
        try {
            $id_token_key = 'client_token' . (is_user_logged_in() ? '_user_' . get_current_user_id() : '');
            $body = null;
            if ($this->enable_tokenized_payments) {
                $paypal_generated_customer_id = $this->ppcp_payment_token->goopter_ppcp_get_paypal_generated_customer_id($this->is_sandbox);
            ...
            $id_token_data = Goopter_Session_Manager::get($id_token_key, null);
            ...
            $args = array(
                ...
                'httpversion' => '1.1',
                'blocking' => true,
                'headers' => ['Content-Type' => 'application/json', 'Authorization' => '', "prefer" => "return=representation", 'PayPal-Request-Id' => $this->generate_request_id(), 'Paypal-Auth-Assertion' => $this->goopter_ppcp_paypalauthassertion()],
                'cookies' => [],
                'body' => $body
            );
            ...
            if (!empty($response['client_token'])) {
                ...
                Goopter_Session_Manager::set($id_token_key, $response);
                return $this->client_token;
            }
    ...
    public function goopter_ppcp_get_generate_id_token() {
        try {
            $body = null;
    ...
    public function goopter_ppcp_regular_create_order_request($woo_order_id = null, $return_url = true) {
        try {
            $return_response = [];
            if (goopter_ppcp_get_order_total($woo_order_id) === 0) {
                $wc_notice = __('Sorry, your session has expired.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
            ...
            $body_request = array(
                'intent' => $intent,
                'application_context' => $this->goopter_ppcp_application_context($return_url),
                'payment_method' => array('payee_preferred' => ($this->payee_preferred) ? 'IMMEDIATE_PAYMENT_REQUIRED' : 'UNRESTRICTED'),
                'purchase_units' =>
                array(
                    0 =>
    ...
    public function goopter_ppcp_error_email_notification($error_email_notification_param, $error_message) {
        if (function_exists('WC')) {
            try {
                ...
                if (!empty($error_email_notification_param['request'])) {
                    $message .= "<strong>" . __('Action: ', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce') . "</strong>" . ucwords(str_replace('_', ' ', $error_email_notification_param['request'])) . PHP_EOL;
                }
                if (!empty($error_message)) {
                    $message .= "<strong>" . __('Error: ', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce') . "</strong>" . $error_message . PHP_EOL;
                }
                if (!empty($error_email_notification_param['order_id'])) {
                    $message .= "<strong>" . __('Order ID: ', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce') . "</strong>" . $error_email_notification_param['order_id'] . PHP_EOL;
                }
                if (is_user_logged_in()) {
                    $userLogined = wp_get_current_user();
                    $message .= "<strong>" . __('User ID: ', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce') . "</strong>" . $userLogined->ID . PHP_EOL;
                    $message .= "<strong>" . __('User Email: ', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce') . "</strong>" . $userLogined->user_email . PHP_EOL;
                }
                $message .= "<strong>" . __('User IP: ', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce') . "</strong>" . WC_Geolocation::get_ip_address() . PHP_EOL;
    ...
    public function goopter_ppcp_update_woo_order_status($orderid, $payment_status, $pending_reason) {
        try {
                ...
                case 'PENDING' :
                    switch (strtoupper($pending_reason)) {
                        case 'BUYER_COMPLAINT':
                            $pending_reason_text = __('BUYER_COMPLAINT: The payer initiated a dispute for this captured payment with PayPal.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
                            break;
                        case 'CHARGEBACK':
                            $pending_reason_text = __('CHARGEBACK: The captured funds were reversed in response to the payer disputing this captured payment with the issuer of the financial instrument used to pay for this captured payment.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
                            break;
                        case 'ECHECK':
                            $pending_reason_text = __('ECHECK: The payer paid by an eCheck that has not yet cleared.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
                            break;
                        case 'INTERNATIONAL_WITHDRAWAL':
                            $pending_reason_text = __('INTERNATIONAL_WITHDRAWAL: Visit your online account. In your **Account Overview**, accept and deny this payment.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
                            break;
                        case 'OTHER':
                            $pending_reason_text = __('No additional specific reason can be provided. For more information about this captured payment, visit your account online or contact PayPal.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
                            break;
                        case 'PENDING_REVIEW':
                            $pending_reason_text = __('PENDING_REVIEW: The captured payment is pending manual review.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
                            break;
                        case 'RECEIVING_PREFERENCE_MANDATES_MANUAL_ACTION':
                            $pending_reason_text = __('RECEIVING_PREFERENCE_MANDATES_MANUAL_ACTION: The payee has not yet set up appropriate receiving preferences for their account. For more information about how to accept or deny this payment, visit your account online. This reason is typically offered in scenarios such as when the currency of the captured payment is different from the primary holding currency of the payee.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
                            break;
                        case 'REFUNDED':
                            $pending_reason_text = __('REFUNDED: The captured funds were refunded.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
                            break;
                        case 'TRANSACTION_APPROVED_AWAITING_FUNDING':
                            $pending_reason_text = __('TRANSACTION_APPROVED_AWAITING_FUNDING: The payer must send the funds for this captured payment. This code generally appears for manual EFTs.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
                            break;
                        case 'UNILATERAL':
                            $pending_reason_text = __('UNILATERAL: The payee does not have a PayPal account.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
                            break;
                        case 'VERIFICATION_REQUIRED':
                            $pending_reason_text = __('VERIFICATION_REQUIRED: The payee\'s PayPal account is not verified.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
                            break;
                        case 'none':
                        default:
                            $pending_reason_text = __('No pending reason provided.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
                            break;
                    }
                    if ($payment_status === 'PENDING') {
                        // Translators: %1$s is the payment method title, %2$s is the PayPal pending reason text.
                        $order->update_status('on-hold', sprintf(__('Payment via %1$s Pending. PayPal Pending reason: %2$s.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), $goopter_ppcp_payment_method_title, $pending_reason_text));
                    }
                    if ($payment_status === 'DECLINED') {
                        // Translators: %1$s is the payment method title, %2$s is the PayPal declined reason text.
                        $order->update_status('failed', sprintf(__('Payment via %1$s declined. PayPal declined reason: %2$s.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), $goopter_ppcp_payment_method_title, $pending_reason_text));
                    }
                    break;
                case 'PARTIALLY_REFUNDED' :
                    $order->update_status('on-hold');
                    // Translators: %1$s is the payment method title, %2$s is the PayPal refund reason.
                    $order->add_order_note(sprintf(__('Payment via %1$s partially refunded. PayPal reason: %2$s.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), $goopter_ppcp_payment_method_title, $pending_reason));
                    break;
                case 'REFUNDED' :
                    $order->update_status('refunded');
                    // Translators: %1$s is the payment method title, %2$s is the PayPal refund reason.
                    $order->add_order_note(sprintf(__('Payment via %1$s refunded. PayPal reason: %2$s.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), $goopter_ppcp_payment_method_title, $pending_reason));
                    break;
                case 'FAILED' :
                    // Translators: %1$s is the payment method title, %2$s is the PayPal failure reason.
                    $order->update_status('failed', sprintf(__('Payment via %1$s failed. PayPal reason: %2$s.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), $goopter_ppcp_payment_method_title, $pending_reason));
                    break;
                case 'VOIDED' :
                    // Translators: %s is the payment method title.
                    $order->update_status('cancelled', sprintf(__('Payment via %s Voided.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), $goopter_ppcp_payment_method_title));
                    break;
                default:
                    break;
            endswitch;
    ...
    public function goopter_ppcp_payment_status_woo_order_note($orderid, $payment_status, $pending_reason) {
        try {
            $order = wc_get_order($orderid);
            switch (strtoupper($payment_status)) :
                case 'DECLINED' :
                    // Translators: %s is the payment method title.
                    $order->add_order_note(sprintf(__('Payment via %s declined.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), $order->get_payment_method_title()));
                case 'PENDING' :
                    switch (strtoupper($pending_reason)) {
                        case 'BUYER_COMPLAINT':
                            $pending_reason_text = __('BUYER_COMPLAINT: The payer initiated a dispute for this captured payment with PayPal.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
                    ...
                    if ($payment_status === 'PENDING') {
                        // Translators: %1$s is the payment method title, %2$s is the PayPal pending reason.
                        $order->add_order_note(sprintf(__('Payment via %1$s Pending. PayPal Pending reason: %2$s.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), $order->get_payment_method_title(), $pending_reason_text));
                    }
                    if ($payment_status === 'DECLINED') {
                        // Translators: %1$s is the payment method title, %2$s is the PayPal declined reason.
                        $order->add_order_note(sprintf(__('Payment via %1$s declined. PayPal declined reason: %2$s.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), $order->get_payment_method_title(), $pending_reason_text));
                    }
                    break;
                case 'PARTIALLY_REFUNDED' :
                    // Translators: %1$s is the payment method title, %2$s is the PayPal refund reason.
                    $order->add_order_note(sprintf(__('Payment via %1$s partially refunded. PayPal reason: %2$s.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), $order->get_payment_method_title(), $pending_reason));
                case 'REFUNDED' :
                    // Translators: %1$s is the payment method title, %2$s is the PayPal refund reason.
                    $order->add_order_note(sprintf(__('Payment via %1$s refunded. PayPal reason: %2$s.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), $order->get_payment_method_title(), $pending_reason));
                case 'FAILED' :
                    // Translators: %1$s is the payment method title, %2$s is the PayPal failure reason.
                    $order->add_order_note(sprintf(__('Payment via %1$s failed. PayPal reason: %2$s.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), $order->get_payment_method_title(), $pending_reason));
                    break;
                default:
                    break;
            endswitch;
    ...
    public function goopter_ppcp_get_paypal_order_details($paypal_order_id) {
        try {
            $args = array(
                'timeout' => 60,
                'redirection' => 5,
    ...
    public function goopter_ppcp_get_authorized_payment($authorization_id) {
        try {
            $args = array(
                'timeout' => 60,
                'redirection' => 5,
    ...
    public function goopter_ppcp_void_authorized_payment_admin($order, $order_data) {
        try {
            $order_id = $order->get_id();
            $note_to_payer = $order_data['goopter_ppcp_note_to_buyer_void'] ?? '';
            if (strlen($note_to_payer) > 255) {
                $note_to_payer = substr($note_to_payer, 0, 252) . '...';
            }
            $authorization_id = goopter_ppcp_get_post_meta($order, '_auth_transaction_id');
            $response = $this->goopter_ppcp_void_authorized_payment($authorization_id, $note_to_payer);
            if (!is_wp_error($response)) {
                $payment_status = $response['status'] ?? '';
                $this->goopter_ppcp_update_woo_order_status($order_id, $payment_status, $pending_reason = '');
            } else {
                $order->add_order_note(__("Void Authorization Failed:", 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce') . ': ' . $response->get_error_message());
            }
        } catch (Exception $ex) {
            $this->api_log->log("The exception was created on line: " . $ex->getFile() . ' ' . $ex->getLine(), 'error');
            $this->api_log->log($ex->getMessage(), 'error');
     ...
     * @return false|void
     */
    public function goopter_ppcp_capture_authorized_payment_admin($order, $order_data) {
        try {
            if ($order === false) {
                return false;
            }
            $note_to_payer = $order_data['goopter_ppcp_note_to_buyer_capture'] ?? '';
            if (strlen($note_to_payer) > 255) {
                $note_to_payer = substr($note_to_payer, 0, 252) . '...';
            }
            $order_id = $order->get_id();
            ...
            $capture_arg = array(
                ...
                array(
                    'value' => $amount_value,
                    'currency_code' => apply_filters('goopter_ppcp_woocommerce_currency', goopter_ppcp_get_currency($order_id), $amount_value),
                ),
                'note_to_payer' => $note_to_payer,
            ...
            $body_request = goopter_ppcp_remove_empty_key($capture_arg);
            ...
            if (!empty($this->api_response['id'])) {
                ...
                $this->api_response = $this->goopter_ppcp_get_authorized_payment($authorization_id);
    ...
    public function goopter_ppcp_refund_order_admin($order, $order_data) {
        try {
            if ($order === false) {
                return false;
            }
            ...
            $reason = !empty($reason) ? $reason : 'Refund';
            $body_request['note_to_payer'] = $reason;
            $amount_value = isset($order_data['_goopter_ppcp_refund_price']) ? goopter_ppcp_round($order_data['_goopter_ppcp_refund_price'], $decimals) : '';
            $body_request['amount'] = array(
                'value' => $amount_value,
                'currency_code' => apply_filters('goopter_ppcp_woocommerce_currency', goopter_ppcp_get_currency($order_id), $amount_value)
            );
            $body_request = goopter_ppcp_remove_empty_key($body_request);
            $transaction_id = $order_data['goopter_ppcp_refund_data'] ?? '';
            $args = array(
                'method' => 'POST',
                'timeout' => 60,
                'redirection' => 5,
    ...
    public function goopter_ppcp_add_payment_source_parameter($request) {
        try {
            $payment_method_name = '';
            $goopter_ppcp_used_payment_method = Goopter_Session_Manager::get('used_payment_method', 'paypal');
            if (!empty($goopter_ppcp_used_payment_method)) {
                $payment_method_name = '';
                $billing_address = array();
                $billing_full_name = '';
                $attributes = array();
                switch ($goopter_ppcp_used_payment_method) {
                    case 'card':
                    ...
                    case 'paypal':
                        ...
                        $request['payment_source'][$payment_method_name]['attributes'] = $attributes;
                        //$request['payment_source'][$payment_method_name]['experience_context']['shipping_preference'] = $this->goopter_ppcp_shipping_preference();
                        if(!isset($request['application_context']['return_url'])) {
                            $request['payment_source'][$payment_method_name]['experience_context']['return_url'] = add_query_arg(array('goopter_ppcp_action' => 'regular_capture', 'utm_nooverride' => '1'), untrailingslashit(WC()->api_request_url('Goopter_PayPal_PPCP_Front_Action')));
                            $request['payment_source'][$payment_method_name]['experience_context']['cancel_url'] = add_query_arg(array('goopter_ppcp_action' => 'regular_cancel', 'utm_nooverride' => '1'), untrailingslashit(WC()->api_request_url('Goopter_PayPal_PPCP_Front_Action')));
                        }
                        break;
                    case 'venmo':
    ...
    public function goopter_ppcp_capture_order_using_payment_method_token($order_id) {
        try {
            // phpcs:disable WordPress.Security.NonceVerification.Missing -- no security issue, paypal token is used to verify
            $order = wc_get_order($order_id);
            $goopter_ppcp_payment_method_title = $this->get_payment_method_title_for_order($order_id);
            $this->paymentaction = apply_filters('goopter_ppcp_paymentaction', $this->paymentaction, $order_id);
            $cart = $this->goopter_ppcp_get_details_from_order($order_id);
            ...
            $body_request = $this->goopter_ppcp_set_payer_details($order_id, $body_request);
            $body_request = apply_filters('goopter_ppcp_add_payment_source', $body_request, $order_id);
            $body_request = goopter_ppcp_remove_empty_key($body_request);
            $args = array(
                'method' => 'POST',
                'headers' => array('Content-Type' => 'application/json', 'Authorization' => '', "prefer" => "return=representation", 'PayPal-Request-Id' => $this->generate_request_id(), 'Paypal-Auth-Assertion' => $this->goopter_ppcp_paypalauthassertion()),
          ...
          //  $args = apply_filters('goopter_ppcp_request_args', $args, 'create_order', $order_id);
            $this->api_response = $this->api_request->request($this->paypal_order_api, $args, 'create_order');
            if (ob_get_length()) {
                ob_end_clean();
            }
            if (isset($this->api_response['id']) && !empty($this->api_response['id'])) {
                        ...
                        if (isset($this->api_response['payment_source']['card']['from_request']['expiry'])) {
                            ...
                            if (!empty($token_id)) {
                                ...
                                if (isset($this->api_response['payment_source']['card']['expiry'])) {
                                    ...
                                    $card_details = $this->goopter_ppcp_get_payment_token_details($token->get_token());
                ...
                $readable_message = $this->goopter_ppcp_get_readable_message($this->api_response, $error_email_notification_param);
                $order->add_order_note($readable_message);
                return false;
            }
            // phpcs:enable WordPress.Security.NonceVerification.Missing -- no security issue, paypal token is used to verify
        } catch (Exception $ex) {
            $this->api_log->log("The exception was created on line: " . $ex->getFile() . ' ' . $ex->getLine(), 'error');
            $this->api_log->log($ex->getMessage(), 'error');
        }
    ...
    public function goopter_ppcp_get_order_return_url($order = null) {
        if ($order) {
    ...
    public function goopter_ppcp_paypal_setup_tokens() {
        try {
            $body_request = array();
            ...
            $body_request['payment_source']['paypal']['experience_context'] = array(
                ...
                'brand_name' => $this->brand_name,
                'locale' => 'en-US',
                'return_url' => add_query_arg(array('goopter_ppcp_action' => 'paypal_create_payment_token', 'utm_nooverride' => '1', 'customer_id' => get_current_user_id()), untrailingslashit(WC()->api_request_url('Goopter_PayPal_PPCP_Front_Action'))),
    ...
    public function goopter_ppcp_paypal_setup_tokens_free_signup_with_free_trial($order_id) {
        try {
            $body_request = array();
            $body_request['payment_source']['paypal']['description'] = "Billing Agreement";
            $body_request['payment_source']['paypal']['permit_multiple_payment_tokens'] = true;
            ...
            $body_request['payment_source']['paypal']['experience_context'] = array(
                ...
                'brand_name' => $this->brand_name,
                'locale' => 'en-US',
                'return_url' => add_query_arg(array('goopter_ppcp_action' => 'paypal_create_payment_token_free_signup_with_free_trial', 'utm_nooverride' => '1', 'customer_id' => get_current_user_id(), 'order_id' => $order_id), untrailingslashit(WC()->api_request_url('Goopter_PayPal_PPCP_Front_Action'))),
        ...
        try {
            ...
            if (isset($_GET[GOOPTER_APPROVAL_TOKEN_ID_PARAM_NAME]) && isset($_GET['order_id'])) {
                ...
                if (!empty($this->api_response['id'])) {
                    ...
                    if (goopter_ppcp_get_token_id_by_token($this->api_response['id']) === '') {
                        ...
                        if ($token->validate()) {
                            ...
                            $order->add_order_note('ERROR MESSAGE: ' . __('Invalid or missing payment token fields.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'));
                        }
                    } else {
                        add_filter('woocommerce_payment_complete_order_status', function ($payment_status, $order_id) {
                            return $this->get_preferred_order_status($payment_status, $order_id);
            ...
            // phpcs:enable WordPress.Security.NonceVerification.Recommended -- no security issue, paypal token is used to verify
        } catch (Exception $ex) {
                ...
                if (!empty($this->api_response['id'])) {
                    ...
                    if (goopter_ppcp_get_token_id_by_token($this->api_response['id']) === '') {
                        ...
                        if ($token->validate()) {
                            $token->save();
                            update_metadata('payment_token', $token->get_id(), '_goopter_ppcp_used_payment_method', 'paypal');
                            wc_add_notice(__('Payment method successfully added.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'));
                        } else {
                            wc_add_notice(__('Unable to add payment method to your account.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), 'error');
                        }
                    } else {
                        wc_add_notice(__('Payment method already exist in your account.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), 'notice');
                    }
    ...
    public function goopter_ppcp_advanced_credit_card_setup_tokens() {
        try {
            $body_request = array();
            $customer = WC()->customer;
            $first_name = $customer->get_billing_first_name();
            ...
            $body_request['payment_source']['card']['experience_context'] = array(
                'brand_name' => $this->brand_name,
                'locale' => 'en-US',
                'return_url' => add_query_arg(array('goopter_ppcp_action' => 'advanced_credit_card_create_payment_token', 'utm_nooverride' => '1', 'customer_id' => get_current_user_id()), untrailingslashit(WC()->api_request_url('Goopter_PayPal_PPCP_Front_Action'))),
                'cancel_url' => wc_get_account_endpoint_url('add-payment-method')
            );
            $paypal_generated_customer_id = $this->ppcp_payment_token->goopter_ppcp_get_paypal_generated_customer_id($this->is_sandbox);
            if (!empty($paypal_generated_customer_id)) {
            ...
            if (isset($_GET[GOOPTER_APPROVAL_TOKEN_ID_PARAM_NAME])) {
                ...
                if (!empty($this->api_response['id'])) {
                    ...
                    if (goopter_ppcp_get_token_id_by_token($this->api_response['id']) === '') {
                        ...
                        $wc_customer_id = get_current_user_id();
                        $token->set_token($this->api_response['id']);
                        $token->set_gateway_id('goopter_ppcp_cc');
                        ...
                        if (isset($this->api_response['payment_source']['card']['expiry'])) {
                            ...
                            $card_details = $this->goopter_ppcp_get_payment_token_details($this->api_response['id']);
                            if (isset($card_details['payment_source']['card']['expiry'])) {
                                $card_expiry = array_map('trim', explode('-', $card_details['payment_source']['card']['expiry']));
                                $card_exp_year = str_pad($card_expiry[0], 4, "0", STR_PAD_LEFT);
                                $card_exp_month = $card_expiry[1] ?? '';
                        ...
                        if ($token->validate()) {
                            ...
                            wc_add_notice(__('Payment method has been added successfully.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'));
                        } else {
                            wc_add_notice(__('Unable to add payment method to your account.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), 'error');
                        }
                    } else {
    ...
    public function goopter_ppcp_advanced_credit_card_setup_tokens_free_signup_with_free_trial($posted_card, $order_id) {
        try {
            $body_request = array();
            $customer = WC()->customer;
            $first_name = $customer->get_billing_first_name();
            ...
            $body_request['payment_source']['card']['experience_context'] = array(
                'brand_name' => $this->brand_name,
                'locale' => 'en-US',
                'return_url' => add_query_arg(array('goopter_ppcp_action' => 'advanced_credit_card_create_payment_token_free_signup_with_free_trial', 'utm_nooverride' => '1', 'customer_id' => get_current_user_id(), 'order_id' => $order_id), untrailingslashit(WC()->api_request_url('Goopter_PayPal_PPCP_Front_Action'))),
                'cancel_url' => wc_get_checkout_url()
            );
    ...
    public function goopter_ppcp_advanced_credit_card_setup_tokens_sub_change_payment($posted_card, $order_id) {
        try {
            $body_request = array();
            $customer = WC()->customer;
            $first_name = $customer->get_billing_first_name();
            ...
            $body_request['payment_source']['card']['experience_context'] = array(
                'brand_name' => $this->brand_name,
                'locale' => 'en-US',
                'return_url' => add_query_arg(array('goopter_ppcp_action' => 'advanced_credit_card_create_payment_token_sub_change_payment', 'utm_nooverride' => '1', 'customer_id' => get_current_user_id(), 'order_id' => $order_id), untrailingslashit(WC()->api_request_url('Goopter_PayPal_PPCP_Front_Action'))),
                'cancel_url' => wc_get_checkout_url()
            );
            $body_request['payment_source']['card']['stored_credential'] = array(
                'payment_initiator' => 'CUSTOMER',
                ...
                if (!empty($this->api_response['id'])) {
                    ...
                    if (goopter_ppcp_get_token_id_by_token($this->api_response['id']) === '') {
                        ...
                        if ($token->validate()) {
                            ...
                            wc_add_notice(__('Unable to change payment method.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), 'error');
                        }
                    }
                    wp_redirect(goopter_ppcp_get_view_sub_order_url($order_id));
                    exit();
    ...
    public function goopter_ppcp_paypal_setup_tokens_sub_change_payment($order_id) {
        try {
            $body_request = array();
            $body_request['payment_source']['paypal']['description'] = "Billing Agreement";
            $body_request['payment_source']['paypal']['permit_multiple_payment_tokens'] = true;
            ...
            $body_request['payment_source']['paypal']['experience_context'] = array(
                ...
                'brand_name' => $this->brand_name,
                'locale' => 'en-US',
                'return_url' => add_query_arg(array('goopter_ppcp_action' => 'paypal_create_payment_token_sub_change_payment', 'utm_nooverride' => '1', 'customer_id' => get_current_user_id(), 'order_id' => $order_id), untrailingslashit(WC()->api_request_url('Goopter_PayPal_PPCP_Front_Action'))),
                'cancel_url' => wc_get_checkout_url()
            );
            $paypal_generated_customer_id = $this->ppcp_payment_token->goopter_ppcp_get_paypal_generated_customer_id($this->is_sandbox);
            if (!empty($paypal_generated_customer_id)) {
    ...
    public function goopter_ppcp_get_all_payment_tokens_for_renewal($user_id) {
        try {
            $paypal_generated_customer_id = $this->ppcp_payment_token->goopter_ppcp_get_paypal_generated_customer_id_for_renewal($this->is_sandbox, $user_id);
    ...
    public function goopter_ppcp_get_all_payment_tokens() {
        try {
            $paypal_generated_customer_id = $this->ppcp_payment_token->goopter_ppcp_get_paypal_generated_customer_id($this->is_sandbox);
            if ($paypal_generated_customer_id === false) {
                return false;
    ...
    public function goopter_ppcp_get_payment_token_details($id) {
        try {
            $args = array(
                'method' => 'GET',
                'headers' => array('Content-Type' => 'application/json', 'Authorization' => '', "prefer" => "return=representation", 'PayPal-Request-Id' => $this->generate_request_id(), 'Paypal-Auth-Assertion' => $this->goopter_ppcp_paypalauthassertion()),
    ...
    public function goopter_ppcp_add_payment_source($body_request, $order_id) {
        try {
            $order = wc_get_order($order_id);
            $user_id = (int) $order->get_customer_id();
            $all_payment_tokens = $this->goopter_ppcp_get_all_payment_tokens_for_renewal($user_id);
            $payment_tokens_id = goopter_ppcp_get_post_meta($order, '_payment_tokens_id', true);
            if (empty($payment_tokens_id)) {
                $payment_tokens_id = goopter_ppcp_get_post_meta($order, 'payment_token_id', true);
            }
            if (empty($payment_tokens_id)) {
                $payment_tokens_id = goopter_ppcp_get_post_meta($order, '_ppec_billing_agreement_id', true);
            }
            $paypal_subscription_id = goopter_ppcp_get_post_meta($order, '_paypal_subscription_id', true);
                ...
                foreach ($all_payment_tokens as $key => $paypal_payment_token) {
                    if ($paypal_payment_token['id'] === $payment_tokens_id) {
                        foreach ($paypal_payment_token['payment_source'] as $type_key => $payment_tokens_data) {
                            ...
                            $goopter_ppcp_payment_method_title = goopter_ppcp_get_payment_method_title($type_key);
                            $order->set_payment_method_title($goopter_ppcp_payment_method_title);
                            $order->update_meta_data('_goopter_ppcp_used_payment_method', $type_key);
                            $order->save();
                            return $body_request;
                        }
                    }
            ...
            $goopter_ppcp_old_payment_method = goopter_ppcp_get_post_meta($order, '_goopter_ppcp_old_payment_method', true);
            if (empty($goopter_ppcp_old_payment_method)) {
                $goopter_ppcp_old_payment_method = goopter_ppcp_get_post_meta($order, '_old_payment_method', true);
            }

            if (!isset($body_request['payment_source'])) {
                if (empty($all_payment_tokens) && !empty($payment_tokens_id)) {
                    $payment_method = goopter_ppcp_get_post_meta($order, '_goopter_ppcp_used_payment_method', true);
        ...
        $goopter_ppcp_payment_method_title = goopter_ppcp_get_payment_method_title($payment_method);
        $order->set_payment_method_title($goopter_ppcp_payment_method_title);
        $order->save();
        return $body_request;
    }
    ...
    public function goopter_ppcp_delete_payment_token($payment_token) {
        try {
            $args = array(
                'method' => 'DELETE',
                'headers' => array('Content-Type' => 'application/json', 'Authorization' => '', "prefer" => "return=representation", 'PayPal-Request-Id' => $this->generate_request_id(), 'Paypal-Auth-Assertion' => $this->goopter_ppcp_paypalauthassertion()),
    ...
    public function goopter_ppcp_get_all_payment_tokens_by_user_id($user_id) {
        try {
            $paypal_generated_customer_id = $this->ppcp_payment_token->goopter_ppcp_get_paypal_generated_customer_id_by_user_id($this->is_sandbox, $user_id);
            if ($paypal_generated_customer_id === false) {
                return false;
            }
            $args = array(
    ...
    public function goopter_ppcp_prepare_refund_request_data_for_capture($order, $amount) {
        try {
            // phpcs:disable WordPress.Security.NonceVerification.Missing -- checked by woocommerce refund process
            ...
            $capture_data_list = $this->goopter_ppcp_get_capture_data_with_line_item_id($order);
                ...
                if ($amount > 0 && isset($capture_data_list[$item_id])) {
                    foreach ($capture_data_list[$item_id] as $transaction_id => $capture_amount) {
                        ...
                        if (!empty($transaction_data)) {
                            ...
                            $closest_amount = goopter_ppcp_binary_search($transaction_data, $refund_to_add);
            ...
            if ($refund_amount > $ppcp_refunded_amount) {
                $capture_data_list = $this->goopter_ppcp_get_capture_data($order);
            ...
            // phpcs:enable WordPress.Security.NonceVerification.Missing -- checked by woocommerce refund process
            return $prepare_refund_data;
        } catch (Exception $ex) {
    ...
    public function goopter_ppcp_get_capture_data_with_line_item_id($order) {
    ...
    public function goopter_ppcp_get_capture_data($order) {
        $capture_data_list = array();
        foreach ($order->get_items(array( 'line_item', 'tax', 'shipping', 'fee', 'coupon'  )) as $item) {
            if ($item->meta_exists('_ppcp_capture_details')) {
                $ppcp_capture_details = $item->get_meta('_ppcp_capture_details');
    ...
    public function goopter_ppcp_refund_capture_order($order_id, $amount, $note_to_payer, $transaction_id, $item_id) {
        try {
            $order = wc_get_order($order_id);
            if ($order === false) {
                return false;
            ...
            if (!empty($amount) && $amount > 0) {
                $body_request['amount'] = array(
                    'value' => goopter_ppcp_round($amount, $decimals),
                    'currency_code' => apply_filters('goopter_ppcp_woocommerce_currency', $currency_code, $amount)
                );
            }
            $args = array(
                'method' => 'POST',
            ...
            if (isset($this->api_response['status'])) {
                ...
                $this->goopter_ppcp_update_capture_details($transaction_id, $refund_transaction_id, $gross_amount, $item_id);
            } else {
                $error_email_notification_param = array(
                    'request' => 'refund_order',
                    'order_id' => $order_id
    ...
    public function goopter_ppcp_get_capture_details($capture_id) {
        try {
            $args = array(
                'timeout' => 60,
                'redirection' => 5,
    ...
    public function goopter_ppcp_sync_ppcp_capture_details($order_id) {
        try {
            $order = wc_get_order($order_id);
            if ($order === false) {
                return false;
            ...
            if (!empty($capture_data_list)) {
                foreach ($capture_data_list as $item_id => $capture) {
                    foreach ($capture as $capture_id => $capture_amount) {
                        $capture_details = $this->goopter_ppcp_get_capture_details($capture_id);
    ...
    public function goopter_ppcp_update_capture_details($capture_id, $refund_id, $refund_amount, $item_id) {
        try {
