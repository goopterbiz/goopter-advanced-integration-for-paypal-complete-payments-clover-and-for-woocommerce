...
if (class_exists("Goopter_WFOCU_Paypal_For_WC_Gateway_PPCP") || !class_exists("WFOCU_Gateway")) {
    return;
}

class Goopter_WFOCU_Paypal_For_WC_Gateway_PPCP extends WFOCU_Gateway {
    ...
    public function __construct() {
        ...
        if ($this->enable_tokenized_payments === false) {
            ...
            add_filter("goopter_ppcp_is_save_payment_method", array($this, "goopter_force_token_save"), 20);
        }
        add_action('wc_ajax_wfocu_front_handle_goopter_ppcp_payments', array($this, 'process_client_order'));
        add_action('woocommerce_api_wfocu_goopter_ppcp_payments', array($this, 'handle_api_calls'));
    ...
    public function replace_form_template($template) {
        ...
        if (isset($_GET['paypal_order_id'])) {
            ...
            $billing_address = goopter_ppcp_get_mapped_billing_address($checkout_details, ($this->set_billing_address) ? false : true);
    ...
    public function has_token($order) {
        try {
            ...
            $this->token = goopter_ppcp_get_post_meta($order, '_payment_tokens_id', true);
            if (!empty($this->token)) {
                return true;
            }
            return false;
        ...
        try {
            ...
            $capture_args = array(
                ...
                'headers' => array(
                    ...
                    'Paypal-Auth-Assertion' => $this->goopter_ppcp_paypalauthassertion($environment),
                ),
            );
            ...
            if (isset($resp_body['status']) && 'failed' === $resp_body['status']) {
                ...
                $resp_body = json_decode(wp_json_encode($resp_body), FALSE);
            ...
            if (true === WFOCU_AJAX_Controller::validate_charge_request($posted_data)) {
                ...
                $data = array(
                    ...
                    'application_context' => array(
                        ...
                        'return_url' => add_query_arg(array('wfocu-si' => WFOCU_Core()->data->get_transient_key()), WC()->api_request_url('wfocu_goopter_ppcp_payments')),
    ...
    public function allow_action($actions) {
        try {
            array_push($actions, 'wfocu_front_handle_goopter_ppcp_payments');
            return $actions;
        } catch (Exception $ex) {
    ...
    public function get_token($order) {
        try {
            $this->token = goopter_ppcp_get_post_meta($order, '_payment_tokens_id', true);
            if (empty($this->token)) {
                $payment_tokens_list = $this->payment_request->goopter_ppcp_get_all_payment_tokens();
                $payment_method = $order->get_payment_method();
                if ($payment_method == 'goopter_ppcp') {
            ...
            if ($this->enable_tokenized_payments) {
                ...
                if (true === WFOCU_AJAX_Controller::validate_charge_request($posted_data)) {
                    ...
                    $data['application_context'] = array(
                        ...
                        'landing_page' => $this->landing_page,
                        'brand_name' => html_entity_decode(get_bloginfo('name'), ENT_NOQUOTES, 'UTF-8'),
                        'return_url' => add_query_arg(array('wfocu-si' => WFOCU_Core()->data->get_transient_key()), WC()->api_request_url('wfocu_goopter_goopter_ppcp_payments')),
                        'cancel_url' => add_query_arg(array('wfocu-si' => WFOCU_Core()->data->get_transient_key()), WFOCU_Core()->public->get_the_upsell_url(WFOCU_Core()->data->get_current_offer())),
                    );
                    ...
                    $token_id = goopter_ppcp_get_token_id_by_token($payment_token);
                    $data_store = WC_Data_Store::load('payment-token');
                    $token_metadata = $data_store->get_metadata($token_id);
                    if ($token_metadata) {
                        $data['payment_source'] = array(
                            $token_metadata['_goopter_ppcp_used_payment_method'][0] => array(
                                'vault_id' => $payment_token,
                            )
                        );
                    } else {
                        $data = apply_filters('goopter_ppcp_add_payment_source', $data, $get_order->get_id());
                    }
                    if ($token_metadata['_goopter_ppcp_used_payment_method'][0] === 'card') {
                        $data['payment_source'][$token_metadata['_goopter_ppcp_used_payment_method'][0]]['stored_credential'] = array(
    ...
    public function goopter_ppcp_paypalauthassertion($environment) {
        try {
            $temp = array(
                "alg" => "none"
            );
    ...
    public function goopter_force_token_save($is_enabled) {
        try {
