...
final class Goopter_Google_Pay_Checkout_Block extends AbstractPaymentMethodType {

    private $gateway;
    protected $name = 'goopter_ppcp_google_pay';
    public $pay_later;
    public $version;
    ...
    public function initialize() {
        ...
        $this->gateway = new Goopter_WC_Gateway_Google_Pay();
    ...
    public function get_payment_method_script_handles() {
        ...
        wp_register_script('goopter_google_pay-blocks-integration', PAYPAL_FOR_WOOCOMMERCE_ASSET_URL . 'ppcp-gateway/checkout-block/ppcp-google-pay.js', array('jquery', 'react', 'wc-blocks-registry', 'wc-settings', 'wp-element', 'wp-i18n', 'wp-polyfill', 'wp-element', 'wp-plugins'), VERSION_PFW, true);
        if (goopter_ppcp_has_active_session()) {
            ...
            $order_button_text = 'Proceed with Google Pay';
        }
        ...
        wp_localize_script('goopter_google_pay-blocks-integration', 'goopter_ppcp_google_pay_manager_block', array(
        ...
        if (function_exists('wp_set_script_translations')) {
            wp_set_script_translations('goopter_google_pay-blocks-integration', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
        }
        if (goopter_ppcp_has_active_session() === false && $page === 'cart') {
            do_action('goopter_ppcp_woo_cart_block_pay_later_message');
        }
        return ['goopter_google_pay-blocks-integration'];
    }

    public function get_payment_method_data() {
        return [
            'title' => $this->get_setting('google_pay_payments_title'),
            'description' => $this->get_setting('google_pay_payments_description'),
