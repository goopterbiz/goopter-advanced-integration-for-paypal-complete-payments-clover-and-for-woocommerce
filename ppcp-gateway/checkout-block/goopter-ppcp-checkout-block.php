...
final class Goopter_PPCP_Checkout_Block extends AbstractPaymentMethodType {

    private $gateway;
    protected $name = 'goopter_ppcp';
    public $pay_later;
    public $version;
    ...
    public function initialize() {
        ...
        $this->gateway = new Goopter_WC_Gateway_PPCP();
    ...
    public function get_payment_method_script_handles() {
        ...
        wp_register_script('goopter_ppcp-blocks-integration', PAYPAL_FOR_WOOCOMMERCE_ASSET_URL . 'ppcp-gateway/checkout-block/ppcp-checkout.js', array('jquery', 'react', 'wc-blocks-registry', 'wc-settings', 'wp-element', 'wp-i18n', 'wp-polyfill', 'wp-element', 'wp-plugins'), VERSION_PFW, true);
        ...
        switch ($payment_method_used) {
            case 'apple_pay':
                $payment_method = 'goopter_ppcp_apple_pay';
                break;
            case 'google_pay':
                $payment_method = 'goopter_ppcp_google_pay';
                break;
            default:
                $payment_method = 'goopter_ppcp';
        }

        wp_localize_script('goopter_ppcp-blocks-integration', 'goopter_ppcp_manager_block', array(
        ...
        if (function_exists('wp_set_script_translations')) {
            wp_set_script_translations('goopter_ppcp-blocks-integration', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
        }
        if (goopter_ppcp_has_active_session() === false && $page === 'cart') {
            do_action('goopter_ppcp_woo_cart_block_pay_later_message');
        }
        return ['goopter_ppcp-blocks-integration'];
    }
