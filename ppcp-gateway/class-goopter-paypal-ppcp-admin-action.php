...
class Goopter_PayPal_PPCP_Admin_Action {
        ...
        if (in_array($payment_method, ['goopter_ppcp_cc', 'goopter_ppcp', 'goopter_ppcp_apple_pay', 'goopter_ppcp_google_pay']) && $transaction_id && $paymentaction === 'authorize') {
            ...
            if ($this->goopter_ppcp_is_authorized_only($trans_details)) {
                ...
                if (!is_wp_error($response)) {
                    $note = __("Authorization Voided", 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
                } else {
                    $note = __("Void Authorization Failed:", 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce') . ': ' . $response->get_error_message();
                }
    ...
    public function admin_notices() {
        try {
            ...
            if (isset($_GET['page']) && 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce' === $_GET['page']) {
                return;
            }
            ...
            if (!empty($goopter_ppcp_account_reconnect_notice) && !empty($notice_type['active_ppcp_gateways']) && isset($notice_type['active_ppcp_gateways']['goopter_ppcp'])) {
                ...
                $notice_data_account_reconnect = array(
                    ...
                    'ans_button_url' => admin_url('options-general.php?page=goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'),
    ...
    public function goopter_ppcp_order_action_list($order_id) {
        try {
            $order = wc_get_order($order_id);
            if (!is_a($order, 'WC_Order')) {
                echo esc_html(__('Error: Unable to detect the order, please refresh again to retry or Contact PayPal For WooCommerce support.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'));
                return;
            }
            $this->order = $order;
            ...
            if (empty($paypal_order_id)) {
                //echo __('PayPal order id does not exist for this order.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
                return;
            }
                ...
                if (isset($this->payment_response['purchase_units']['0']['payments']['authorizations']) && !empty($this->payment_response['purchase_units']['0']['payments']['authorizations'])) {
                    if (isset($this->payment_response['purchase_units']['0']['payments']['refunds'])) {
                        foreach ($this->payment_response['purchase_units']['0']['payments']['refunds'] as $key => $refunds) {
                            ...
                            $line_item['payment_action'] = __('Refund', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
                    ...
                    if (isset($this->payment_response['purchase_units']['0']['payments']['captures'])) {
                        foreach ($this->payment_response['purchase_units']['0']['payments']['captures'] as $key => $captures) {
                            ...
                            $line_item['payment_action'] = __('Capture', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
                    ...
                    if (isset($this->payment_response['purchase_units']['0']['payments']['authorizations']['0']['status']) && 'CREATED' === $this->payment_response['purchase_units']['0']['payments']['authorizations']['0']['status']) {
                        $this->goopter_ppcp_order_actions['void'] = __('Void Authorization', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
                        $this->goopter_ppcp_order_actions['capture'] = __('Capture Funds', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
                    }
                    if (isset($this->payment_response['purchase_units']['0']['payments']['authorizations']['0']['status']) && 'PARTIALLY_CAPTURED' === $this->payment_response['purchase_units']['0']['payments']['authorizations']['0']['status']) {
                        if ($this->goopter_refund_amount < $this->goopter_capture_amount) {
                            $this->goopter_ppcp_order_actions['refund'] = __('Refund', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
                            $this->goopter_ppcp_order_actions['void'] = __('Void Authorization', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
                            $this->goopter_ppcp_order_actions['capture'] = __('Capture Funds', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce');
                        }
    ...
    public function goopter_ppcp_add_order_action_buttons($order) {
        try {
            ...
            if ($should_display_transaction_details && !empty($this->goopter_ppcp_order_actions) && $this->is_auto_capture_auth === false) {
                ...
                <button type="button"
                        class="button goopter-ppcp-order-capture" <?php echo (isset($this->goopter_ppcp_order_actions['capture']) && !empty($this->goopter_ppcp_order_actions)) ? '' : 'disabled'; ?>> <?php esc_html_e('Capture', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'); ?><?php echo wp_kses_post(wc_help_tip(__('Capture payment for the authorized order.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'))); ?></button>
                <button type="button"
                        class="button goopter-ppcp-order-void" <?php echo (isset($this->goopter_ppcp_order_actions['void']) && !empty($this->goopter_ppcp_order_actions)) ? '' : 'disabled'; ?>><?php esc_html_e('Void Authorization', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'); ?><?php echo wp_kses_post(wc_help_tip(__('Void the authorized order to release the hold on the buyer\'s payment source.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'))); ?></button>
            <?php }
            ?>
            <?php
        } catch (Exception $ex) {
    ...
    public function goopter_ppcp_order_meta_auth_capture_html() {
        ...
        <?php if (isset($this->goopter_ppcp_order_status_data['capture']) && isset($this->goopter_ppcp_order_actions['capture'])) { ?>
            <tr class="goopter_ppcp_capture_box" style="display: none;">
                <td class="label"><?php echo esc_html(__('Additional Capture Possible', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce')); ?></td>
                <td width="1%"></td>
                <td class="total">
                    <fieldset>
                        <label for="additional_capture_yes">
                            ...
                            Yes
                            <?php echo wp_kses_post(wc_help_tip(__('Yes (option to capture additional funds on this authorization if need)', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'))); ?>
                        </label>
                        <label for="additional_capture_no">
                            <input type="radio" name="additionalCapture" value="no" id="additional_capture_no">
                            No
                            <?php echo wp_kses_post(wc_help_tip(__('No (no additional capture needed; close authorization after this capture)', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'))); ?>
                        </label>
                    </fieldset>
                </td>
            </tr>
            <tr class="goopter_ppcp_capture_box" style="display: none;">
                <td class="label">
                    <label for="refund_amount">
                        <?php echo wp_kses_post(wc_help_tip(__('This will show the total amount to be captured/voided', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'))); ?>
                        <?php esc_html_e('Amount', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'); ?>:
                    </label>
                </td>
                <td width="1%"></td>
                <td class="total">
        ...
        <?php if (isset($this->goopter_ppcp_order_status_data['refund']) && isset($this->goopter_ppcp_order_actions['refund'])) { ?>
            <tr class="goopter_ppcp_refund_box" style="display: none;">
                <td class="label"><?php echo esc_html(__('Transaction Id', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce')); ?></td>
                <td width="1%"></td>
                <td class="total">
                    <select name="goopter_ppcp_refund_data" id="goopter_ppcp_refund_data" style="width: 250px;">
                        ...
                        foreach ($this->goopter_ppcp_order_status_data['refund'] as $k => $v) :
                            if ($i == 0) {
                                echo "<option value=''>" . esc_html(__('Select Transaction Id', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce')) . "</option>";
                            }
                            ?>
            ...
            <tr class="goopter_ppcp_refund_box" style="display: none;">
                <td class="label"><?php echo esc_html(__('Refund Amount', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce')); ?></td>
                <td width="1%"></td>
                <td class="total">
                    <fieldset>
            ...
            foreach ($ppcp_capture_details as $index => $meta_array) :
                ...
                <tr>
                    <td>
                        ...
                        echo '<b>' . esc_html(__('Capture Details', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce')) . '</b>: ';
                            ...
                            foreach ($meta_array as $key => $value) {
                                if (!is_array($value)) {
                                    if ($key === '_ppcp_transaction_date') {
                                        ...
                                        $capture_details_html .= esc_html(sprintf(__('%1$s at %2$s', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), date_i18n(wc_date_format(), strtotime($value)), date_i18n(wc_time_format(), strtotime($value))));
            ...
            foreach ($ppcp_refund_details as $index => $meta_array) :
                ...
                <tr>
                    <td>
                        ...
                        echo '<b>' . esc_html(__('Refund Details', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce')) . '</b>: ';
                        $refund_details_html = '';
                        if (is_array($meta_array) && !empty($meta_array)) {
                            ...
                            foreach ($meta_array as $key => $value) {
                                if ('_ppcp_refund_amount' === $key) {
                                    ...
                                    // Translators: %1$s is the date, and %2$s is the time.
                                    $refund_details_html .= wp_kses_post(sprintf(__('%1$s at %2$s', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'), date_i18n(wc_date_format(), strtotime($value)), date_i18n(wc_time_format(), strtotime($value))));
    ...
    public function goopter_ppcp_display_total_capture($order_id) {
        ...
        <tr>
            <td class="label">
                <?php esc_html_e('Total Capture:', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce'); ?>
            </td>
            <td width="1%"></td>
            <td class="total">
    ...
    public function goopter_ppcp_display_payment_authorization_notice() {
        ...
        if ($screen && $this->goopter_ppcp_is_display_paypal_transaction_details($order->get_id())) {
            echo wp_kses_post('<div class="updated woocommerce-message"><p>' . esc_html__('Capture the authorized order to receive funds in your PayPal account.', 'goopter-advanced-integration-for-paypal-complete-payments-clover-and-for-woocommerce') . '</p></div>');
        }
    }
